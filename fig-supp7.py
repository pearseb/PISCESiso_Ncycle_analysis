...
p1 = plt.contourf(lonproj,latproj, diff_d15nno3_ez, levels=levs1, cmap=colmap, vmin=np.min(levs1), vmax=np.max(levs1), extend='both')
c1 = plt.contour(lonproj,latproj, diff_d15nno3_ez, colors='k', linewidths=wid, levels=np.array([-0.2,0.2]))
...
proj.drawcoastlines(linewidth=0.5, color='k')
proj.fillcontinents(color='grey')
p4 = plt.contourf(lonproj,latproj, diff_d15nno3_pic_ez, levels=levs1, cmap=colmap, vmin=np.min(levs1), vmax=np.max(levs1), extend='both')
c4 = plt.contour(lonproj,latproj, diff_d15nno3_pic_ez, colors='k', linewidths=wid, levels=np.array([-0.2,0.2]))
proj.drawparallels(range(domain_draw[0], domain_draw[2]+1, dlat), labels=[False,True,False,False], color=(.3,.3,.3), linewidth=0, fontsize=fstic)
...
proj.drawcoastlines(linewidth=0.5, color='k')
proj.fillcontinents(color='grey')
p7 = plt.contourf(lonproj,latproj, diff_d15nno3_cc_ez, levels=levs1, cmap=colmap, vmin=np.min(levs1), vmax=np.max(levs1), extend='both')
c7 = plt.contour(lonproj,latproj, diff_d15nno3_cc_ez, colors='k', linewidths=wid, levels=np.array([-0.2,0.2]))
...
proj.drawcoastlines(linewidth=0.5, color='k')
proj.fillcontinents(color='grey')
p2 = plt.contourf(lonproj,latproj, toe_futndep_d15nno3_ez, hatches=hatc, levels=levs1, cmap=colmap_toe, vmin=np.min(levs1), vmax=np.max(levs1), extend='both')
...
proj.drawcoastlines(linewidth=0.5, color='k')
proj.fillcontinents(color='grey')
p5 = plt.contourf(lonproj,latproj, toe_picndep_d15nno3_ez, hatches=hatc, levels=levs1, cmap=colmap_toe, vmin=np.min(levs1), vmax=np.max(levs1), extend='both')
...
proj.drawcoastlines(linewidth=0.5, color='k')
proj.fillcontinents(color='grey')
p8 = plt.contourf(lonproj,latproj, toe_fut_d15nno3_ez, hatches=hatc, levels=levs1, cmap=colmap_toe, vmin=np.min(levs1), vmax=np.max(levs1), extend='both')
...
plt.subplots_adjust(wspace=0.11, left=0.12, bottom=0.075, top=0.875, right=0.95)

levs1 = np.arange(-1,1.01,0.05)

cbax1 = fig.add_axes([0.13, 0.91, 0.38, 0.03])
cbar1 = plt.colorbar(p1, cax=cbax1, orientation='horizontal', ticks=levs1[::4])
cbar1.ax.set_xlabel('21$^{st}$ century $\Delta$ $\delta^{15}$N$^{EZ}_{NO3}$ (\u2030)', fontsize=fslab, labelpad=10)
cbar1.ax.tick_params(labelsize=fstic)
cbar1.ax.xaxis.set_label_position('top')

levs1 = np.concatenate((np.array([1930]), np.arange(1940,2091,10), np.array([2100])))

cbax2 = fig.add_axes([0.565, 0.91, 0.38, 0.03])
...
plt.text(xx,yy, 'c)', fontsize=fslab+2, ha='center', va='center', transform=ax4.transAxes, color=col)
plt.text(xx,yy, 'd)', fontsize=fslab+2, ha='center', va='center', transform=ax5.transAxes, color=col)
plt.text(xx,yy, 'e)', fontsize=fslab+2, ha='center', va='center', transform=ax7.transAxes, color=col)
plt.text(xx,yy, 'f)', fontsize=fslab+2, ha='center', va='center', transform=ax8.transAxes, color=col)
...
fig.savefig('fig-supp7.png', dpi=300, bbox_inches='tight')
fig.savefig('fig-supp7_trans.png', dpi=300, bbox_inches='tight', transparent=True)
